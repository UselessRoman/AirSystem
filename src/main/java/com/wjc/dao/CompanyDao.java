...
public interface CompanyDao {
    ...
    //航司的总里程数
    @Select("select  company.* ,sum(distance) as totalDistance" +
            " from flight join company " +
            "on flight.companyID= company.companyID " +
            "group by  Company.companyID")
    List<Company> findTotalDistance();


    @Select("select company.*,AVG(distance) as averageDistance " +
            "from flight join company " +
    ...
    @Select("select company.* , COUNT(*) as totalPassenger , SUM(Price*discount) as totalSale " +
